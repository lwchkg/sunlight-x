...
function parseCustomTokens(context) {
  ...
  for (tokenName in context.language.customTokens) {
    token = util.matchWord(context, context.language.customTokens[tokenName], tokenName);
...
function getScopeReaderFunction(scope, tokenName) {
  ...
  var closer = typeof scope[1] === 'string' ? new RegExp(util.regexEscape(scope[1])) : scope[1].regex;
