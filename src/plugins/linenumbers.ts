...
import { AfterHighlightNodeEvent } from "../events";
import { globalOptions } from "../globalOptions";
import { warnInvalidValue } from "../logger";
import * as util from "../util";
import { document } from "../jsdom";
import type { AfterHighlightNodeEventArgs } from "../events";
 ...
 */
function getLineNumberOptions(options: SunlightOptionsType): LineNumberOptionsType {
  const logger = console;
  const parsedOptions: LineNumberOptionsType = Object.assign({}, defaultLineNumberOptions);
  if (typeof options.lineNumbers === "boolean") parsedOptions.lineNumbers = options.lineNumbers;else warnInvalidValue("Option lineNumbers must be boolean.", options.lineNumbers);
  if ( // TODO: remove typeof check when Flow issue #4441 is resolved.
  typeof options.lineNumberStart === "number" && Number.isInteger(options.lineNumberStart) && options.lineNumberStart >= 0) parsedOptions.lineNumberStart = options.lineNumberStart;else warnInvalidValue("Option lineNumberStart must be a non-negative integer.", options.lineNumberStart);

  if (Array.isArray(options.lineHighlight)) {
    // TODO: remove typeof check when Flow issue #4441 is resolved.
    for (const line: unknown of options.lineHighlight) if (typeof line === "number" && Number.isInteger(line) && line >= 0) {
...
function getLineCount(node: Element): number {
  ...
  const lastTextNode = function getLastNode(node: Node): Node | null | undefined {
 ...
 */
function maybeAddLineNumbers(highlighter: Highlighter, context: AfterHighlightNodeEventArgs) {
  const codeContainer = context.codeContainer;
  if (!codeContainer) return;
  const options: LineNumberOptionsType = getLineNumberOptions(highlighter.options);
  ...
  for (let i = options.lineNumberStart; i <= options.lineNumberStart + lineCount; ++i) {
    const link = document.createElement("a");
    const name = (context.node.id ? context.node.id : highlighter.options.classPrefix + context.count) + "-line-" + i;
