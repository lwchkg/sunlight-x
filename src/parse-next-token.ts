...
function parseKeyword(context: ParserContext): Token | null | undefined {
  return util.matchWord(context, context.language.keywords, "keyword");
}

function parseCustomTokens(context: ParserContext): Token | null | undefined {
  for (const tokenName in context.language.customTokens) {
    const token = util.matchWord(context, context.language.customTokens[tokenName], tokenName);
...
function parseOperator(context: ParserContext): Token | null | undefined {
  return util.matchWord(context, context.language.operators, "operator");
}

function parsePunctuation(context: ParserContext): Token | null | undefined {
  const current = context.reader.peek();
  if (context.language.punctuation.test(util.regexEscape(current))) return context.createToken("punctuation", context.reader.read());
  return null;
}

function parseIdent(context: ParserContext): Token | null | undefined {
  if (!context.language.identFirstLetter.test(context.reader.peek())) return null;
  let ident = context.reader.read();

  while (!context.reader.isEOF() && context.language.identAfterFirstLetter.test(context.reader.peek())) ident += context.reader.read();

  return context.createToken("ident", ident);
}

function parseDefault(context: ParserContext): Token | null | undefined {
  context.defaultData.text += context.reader.read();
  return null;
}

function parseScopes(context: ParserContext): Token | null | undefined {
...
function parseNumber(context: ParserContext): Token | null | undefined {
  return context.language.numberParser(context);
}

function parseCustomRules(context: ParserContext): (Token | null | undefined) | Token[] {
...
export function parseNextToken(context: ParserContext): (Token | null | undefined) | Token[] {
  if (context.language.doNotParse.test(context.reader.peek())) return parseDefault(context);
