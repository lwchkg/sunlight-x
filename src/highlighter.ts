...
import { DEFAULT_LANGUAGE, TEXT_NODE } from "./constants";
import { globalOptions } from "./globalOptions";
import { BeforeHighlightNodeEvent, AfterHighlightNodeEvent, BeforeHighlightEvent, AfterHighlightEvent, BeforeAnalyzeEvent, AfterAnalyzeEvent } from "./events";
import { languages } from "./languages";
import { AnalyzerContext } from "./analyzer-context";
import { Tokenize } from "./parser-context";
import { error as logError, errorInvalidValue } from "./logger";
import { UserDefinedNameStore } from "./user-defined-name-store";
import { document, window } from "./jsdom";
import type { SunlightOptionsType, SunlightPartialOptionsType } from "./globalOptions";
 ...
 * @param {Node[]} children
 */
function appendAll<T extends Node | Element>(parent: T, children: T[]) {
...
export class Highlighter {
  ...
  constructor(options: SunlightPartialOptionsType | null | undefined) {
  ...
  switchToEmbeddedLanguageIfNecessary(context: ParserContext) {
    for (let i = 0; i < context.language.embeddedLanguages.length; i++) {
      if (!languages[context.language.embeddedLanguages[i].language]) // unregistered language
        continue;
      const embeddedLanguage = util.clone(context.language.embeddedLanguages[i]);
  ...
  analyze(analyzerContext: AnalyzerContext, startIndex: number) {
    ...
    if (analyzerContext.tokens.length > 0) {
      ...
      for (let i = startIndex; i < analyzerContext.tokens.length; i++) {
        const language = languages[analyzerContext.tokens[i].language] || languages[DEFAULT_LANGUAGE];
        ...
        if (![...analyzerContext.analyzerOverrides, analyzerContext.language.analyzer].some((analyzer: Analyzer): boolean => {
          if (Object.prototype.hasOwnProperty.call(analyzer.handlers, tokenName)) {
  ...
  _highlightText(unhighlightedCode: string, languageId: string, userDefinedNameStore: UserDefinedNameStore, partialContext: AnalyzerContext | null | undefined): AnalyzerContext {
    let language = languages[languageId];
    if (language === undefined) // use default language if one wasn't specified or hasn't been registered
    ...
    const analyzerContext = new AnalyzerContext(Tokenize(this, unhighlightedCode, language, userDefinedNameStore, partialContext, this.options), userDefinedNameStore, partialContext, this.options);
    this.analyze(analyzerContext, partialContext && partialContext.index ? partialContext.index + 1 : 0);
    AfterHighlightEvent.raise(this, {
      analyzerContext: analyzerContext
    });
  ...
  getLanguageFromNode(node: Element): string | null | undefined {
    const classPrefix = this.options.classPrefix + "highlight-";
    const classList = node.className.split(" ");

    for (const classItem of classList) if (classItem.startsWith(classPrefix)) return classItem.substring(classPrefix.length);
  ...
  highlight(code: string, languageId: string): AnalyzerContext {
    return this._highlightText(code, languageId, new UserDefinedNameStore(), undefined);
  }
   ...
   * omit == true?)
   */
  highlightNode(node: Element, addContainer: boolean | null | undefined) {
    ...
    BeforeHighlightNodeEvent.raise(this, {
      node: node
    });
    let currentNodeCount = 0;
    const userDefinedNameStore = new UserDefinedNameStore();
    let partialContext: AnalyzerContext | null | undefined = undefined;

    for (const childNode of node.childNodes) if (childNode.nodeType === TEXT_NODE) {
      // text nodes
      ...
      for (let k = 1; k < nodes.length; k++) node.insertBefore(nodes[k], nodes[k - 1].nextSibling);
