...
import { defaultAnalyzer, defaultNumberParser } from "./default-helpers";
import type { Analyzer } from "./analyzer";
import type { AnalyzerContext } from "./analyzer-context";
import type { ParserContext } from "./parser-context";
import type { Token } from "./token";
export type { ParserContext, Token };
export type ScopeType = [string, string | {
  length: number;
  regex: RegExp;
}, string[], boolean];
export type ContextItemsType = Record<string, unknown | unknown[]>;
export type EmbeddedLanguageDefinitionPrecompiled = {
  switchTo: (arg0: ParserContext) => boolean;
  switchBack: (arg0: ParserContext) => boolean;
};
export type EmbeddedLanguageListPrecompiled = Record<string, EmbeddedLanguageDefinitionPrecompiled>;
...
export type CustomIdentRule = {
  (arg0: AnalyzerContext): boolean;
};
export type CustomParseRule = {
  (arg0: ParserContext): (Token | null | undefined) | Token[];
};
export type HashMapType = Record<string, {
  value: string;
  regex: RegExp;
}[]>;
export type Language = {
  name: string;
  analyzer: Analyzer;
  customTokens: Record<string, HashMapType>;
  ...
  punctuation: RegExp;
  numberParser: (arg0: ParserContext) => Token | null | undefined;
 ...
 *                   word.
 */
export function createHashMap(wordMap: string[], boundary: string, caseInsensitive: boolean = false): HashMapType {
  ...
  for (let i = 0; i < wordMap.length; i++) {
    ...
    newMap[firstChar].push({
      ...
      regex: caseInsensitive ? new RegExp("^" + regexEscape(word) + boundary, "i") : new RegExp("^" + regexEscape(word) + boundary)
    });
  }
...
export type LanguageBeforeCompile = {
  ...
  doNotParse?: RegExp;
  contextItems?: ContextItemsType;
  embeddedLanguages?: Record<string, EmbeddedLanguageDefinitionBeforeCompile>;
  customParseRules?: CustomParseRule[];
  scopes?: Record<string, ScopeType[]>;
  identFirstLetter?: RegExp;
  identAfterFirstLetter?: RegExp;
...
export function registerLanguage(languageData: LanguageBeforeCompile) {
  ...
  for (const tokenName in languageData.customTokens) parsedLanguage.customTokens[tokenName] = createHashMap(languageData.customTokens[tokenName].values ? languageData.customTokens[tokenName].values : [], languageData.customTokens[tokenName].boundary, languageData.caseInsensitive);

  // Convert the embedded language object to an easier-to-use array.
  const embeddedLanguages: EmbeddedLanguageDefinition[] = [];

  for (const languageName in languageData.embeddedLanguages) embeddedLanguages.push({
