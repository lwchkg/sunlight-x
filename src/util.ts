...
import type { FollowsOrPrecedesIdentRule, HashMapType } from "./languages";
 ...
 * @returns {boolean}
 */
export function contains<T>(arr: T[], value: T, caseInsensitive: boolean = false): boolean {
  if (!caseInsensitive) return arr.indexOf(value) >= 0;
  return arr.some((element: T): boolean => element === value || caseInsensitive && typeof element === "string" && typeof value === "string" && element.toUpperCase() === value.toUpperCase());
}
 ...
 */
export function matchWord(context: ParserContext, wordMap: HashMapType | null | undefined, tokenName: string, doNotRead: boolean = false): Token | null | undefined {
  wordMap = wordMap || {};
  ...
  const index = subMap.findIndex((wordItem: any): boolean => {
...
export { createHashMap } from "./languages";
 ...
 * @returns {string}
 */
export function nonCapturingGroup(expressions: string | string[], quantifier: string = ""): string {
  const expression = typeof expressions === "string" ? expressions : expressions.join("|");
 ...
 * @returns {function}
 */
export function getRegexpParser(tokenName: string, regex: RegExp): (arg0: ParserContext) => Token | null | undefined {
 ...
 *                     returns a boolean.
 */
export function createBetweenRule(startIndex: number, opener: {
  token: string;
  values: string[];
}, closer: {
  token: string;
  values: string[];
}, caseInsensitive: boolean = false): (arg0: Token[]) => boolean {
 ...
 */
export function createProceduralRule(startIndex: number, direction: number, tokenRequirements: FollowsOrPrecedesIdentRule, caseInsensitive: boolean = false): (arg0: Token[]) => boolean {
  ...
  return function (tokens: Token[]): boolean {
    ...
    for (let j = 0; j < tokenRequirements.length; j++) {
      ...
      if (actual === undefined) {
        if (expected.optional === true) tokenIndexStart -= direction;else return false;
      } else if (actual.name === expected.token && (expected.values === undefined || contains(expected.values, actual.value, caseInsensitive))) // derp
        continue;else if (expected.optional === true) tokenIndexStart -= direction;else // we need to reevaluate against this token again
        return false;
    }
...
type Matcher = (arg0: Token) => boolean;
 ...
 */
function getNextWhileInternal(tokens: Token[], index: number, direction: 1 | -1, matcher: Matcher) {
  direction = direction || 1;
  let count = 1;
  let token;

  while (token = tokens[index + direction * count++]) if (!matcher(token)) return token;
 ...
 * @returns {Token?} The token or undefined
 */
export function getNextWhile(tokens: Token[], index: number, matcher: Matcher): Token | null | undefined {
 ...
 * @returns {Token?} The token or undefined
 */
export function getNextNonWsToken(tokens: Token[], index: number): Token | null | undefined {
  return getNextWhileInternal(tokens, index, 1, (token: Token): boolean => token.name === "default");
}
 ...
 */
export function getPreviousNonWsToken(tokens: Token[], index: number): Token | null | undefined {
  return getNextWhileInternal(tokens, index, -1, (token: Token): boolean => token.name === "default");
}
 ...
 * @returns {Object} The token or undefined
 */
export function getPreviousWhile(tokens: Token[], index: number, matcher: Matcher): Token | null | undefined {
  return getNextWhileInternal(tokens, index, -1, matcher);
}
...
export function getComputedStyle(element: Element, style: string): string {
  let func = null;
  if (document.defaultView && document.defaultView.getComputedStyle) func = document.defaultView.getComputedStyle;else func = function (element: Element): any {
...
export { IsPrecedesRuleSatisfied, IsFollowsRuleSatisfied, IsBetweenRuleSatisfied } from "./rules-processor";
...
export type { BetweenIdentRule } from "./languages";