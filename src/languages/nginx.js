...
export const scopes = {
  ...
  comment: [["#", "\n", [], true]],
  variable: [["$", { length: 1, regex: /[\W]/ }, [], true]],
  label: [["@", { length: 1, regex: /[\W]/ }, [], true]],
...
export const customParseRules = [
  ...
  (function(): ParserContext => ?Token {
    const words = util.createHashMap(
      [
        "server",
        "location",
        "upstream",
  ...
  function(context: ParserContext): ?Token {
    ...
    while (walker.hasPrev()) {
      ...
      if (
        ...
        const prevToken = util.getPreviousWhile(
          context.getAllTokens(),
          walker.index,
          (token: Token): boolean =>
            token.name === "default" || token.name === "comment"
        );

        if (
          prevToken &&
          prevToken.name === "context" &&
          prevToken.value === "location"
        ) {
          isRegexLiteral = true;
          break;
        }
  ...
  (function(): ParserContext => ?Token {
    const directives = util.createHashMap(
      [
        ...
        "gzip_static",
        "gzip_http_version",
        "gzip_proxied" /* "gzip_disable", "gzip_vary",*/, // <-- already accounted for in gzip module
        ...
        "proxy_timeout",
        "proxy",
        "xclient", // <- covered in http ssl module

        // ssl: http://wiki.nginx.org/MailSslModule
        /* "ssl","ssl_certificate","ssl_certificate_key","ssl_ciphers","ssl_prefer_server_ciphers","ssl_protocols","ssl_session_cache","ssl_session_timeout",*/ "starttls",
