...
import type { AnalyzerContext, BetweenIdentRule, FollowsOrPrecedesIdentRule, ParserContext, Token } from "../util";
export const name = "scala";
export const keywords = ["abstract", "case", "catch", "class", "def", "do", "else", "extends", "false", "final", "finally", "forSome", "for", "if", "implicit", "import", "lazy", "match", "new", "null", "object", "override", "package", "private", "protected", "return", "sealed", "super", "this", "throw", "trait", "try", "true", "type", "val", "var", "while", "with", "yield"];
export const embeddedLanguages = {
  xml: {
    switchTo: function (context: ParserContext): boolean {
      ...
      if (context.defaultData.text !== "") // preceded by whitespace
        return true;
    ...
    switchBack: function (context: ParserContext): boolean {
      ...
      if (prevToken.name === "tagName") {
        ...
        if (typeof context.items.literalXmlOpenTag !== "string") context.items.literalXmlOpenTag = prevToken.value;
      ...
      if (typeof context.items.literalXmlOpenTag === "string" && context.items.literalXmlNestingLevel === 0 && (prevToken.value === ">" || prevToken.value === "/>")) return true;
      return false;
    }
  }
};
...
function (context: ParserContext): Token | null | undefined {
  ...
  while (!context.reader.isEOF() && /\w/.test(context.reader.peek())) ident += context.reader.read();

  context.userDefinedNameStore.addName(ident, name);
  return context.createToken("ident", ident);
}];
export const namedIdentRules = {
  custom: [// some built in types
  function (): (arg0: AnalyzerContext) => boolean {
    const builtInTypes = ["Nil", "Nothing", "Unit", "Pair", "Map", "String", "List", "Int", "Seq", "Option", "Double", "AnyRef", "AnyVal", "Any", "ScalaObject", "Float", "Long", "Short", "Byte", "Char", "Boolean"];
  ...
  function (context: AnalyzerContext): boolean {
    ...
    return function (context: AnalyzerContext): boolean {
      ...
      return follows.some((rule: FollowsOrPrecedesIdentRule): boolean => util.IsFollowsRuleSatisfied(context.getTokenWalker(), rule, false)) || between.some((rule: BetweenIdentRule): boolean => util.IsBetweenRuleSatisfied(context.getTokenWalker(), rule.opener, rule.closer, false));
    };
  }()]
};
...
events.BeforeHighlightEvent.addListener((highlighter: Highlighter, context: events.BeforeHighlightEventArgs) => {
