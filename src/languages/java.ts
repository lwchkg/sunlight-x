...
import type { AnalyzerContext, BetweenIdentRule, FollowsOrPrecedesIdentRule } from "../util";
...
const primitives = ["boolean", "byte", "char", "double", "float", "int", "long", "short"];
const acceptableKeywords = primitives.concat(["extends"]);
export const name = "java";
export const keywords = [// http://download.oracle.com/javase/tutorial/java/nutsandbolts/_keywords.html
"abstract", "assert", "boolean", "break", "byte", "case", "catch", "char", "class", "const", "continue", "default", "do", "double", "else", "enum", "extends", "final", "finally", "float", "for", "goto", "if", "implements", "import", "instanceof", "int", "interface", "long", "native", "new", "package", "private", "protected", "public", "return", "short", "static", "strictfp"
/* wtf? */
, "super", "switch", "synchronized", "this", "throw", "throws", "transient", "try", "void", "volatile", "while", // literals
"null", "false", "true"];
...
export const namedIdentRules = {
  custom: [// generic definitions/params between "<" and ">"
  ...
  createNamedIdentFunction((context: AnalyzerContext): boolean => {
    ...
    // also a big fail if it is preceded by a ., i.e. a generic method invocation like container.Resolve()
    {
      ...
      if (token && (token.name === "ident" || token.name === "keyword" && util.contains(primitives.concat(["void"]), token.value) || token.name === "operator" && token.value === ".")) return false;
    }
  ...
  createNamedIdentFunction((context: AnalyzerContext): boolean => {
    ...
    }, util.whitespace], // method/property return values
    // class/interface names
    [{
      token: "keyword",
      values: ["class", "interface", "enum", "public", "private", "protected", "static", "final"]
    }, util.whitespace], // bounded generic interface constraints
    ...
    return precedes.some((rule: FollowsOrPrecedesIdentRule): boolean => util.IsPrecedesRuleSatisfied(context.getTokenWalker(), rule, false)) || follows.some((rule: FollowsOrPrecedesIdentRule): boolean => util.IsFollowsRuleSatisfied(context.getTokenWalker(), rule, false)) || between.some((rule: BetweenIdentRule): boolean => util.IsBetweenRuleSatisfied(context.getTokenWalker(), rule.opener, rule.closer, false));
  })]
};
