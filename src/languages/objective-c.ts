...
export const customParseRules = [// message destination (e.g. method calls)
...
function (): (arg0: ParserContext) => Token | null | undefined {
  const attributes = util.createHashMap(["getter", "setter", "readonly", "readwrite", "assign", "retain", "copy", "nonatomic"], "\\b");
...
export const namedIdentRules = {
  custom: [// naming convention: NS.+, CG.+ are assumed to be built in objects
  ...
  function (context: AnalyzerContext): boolean {
    if (!util.IsPrecedesRuleSatisfied(context.getTokenWalker(), [{
      token: "default"
    }, {
      token: "ident"
    }])) return false;
  ...
  function (): (arg0: AnalyzerContext) => boolean {
    ...
    return function (context: AnalyzerContext): boolean {
      ...
      if (!precedes.some((rule: FollowsOrPrecedesIdentRule): boolean => util.IsPrecedesRuleSatisfied(context.getTokenWalker(), rule, false))) return false;
  ...
  function (context: AnalyzerContext): boolean {
    ...
    while (walker.hasPrev()) {
      ...
      if ( // (token.name === "keyword" && util.contains(acceptableKeywords, token.value))
    ...
    while (walker2.hasNext()) {
      ...
      if ( // TODO: check commented code.
      // (token.name === "keyword" && util.contains(acceptableKeywords, token.value))
      token.name === "operator" && util.contains(["<", "<<", "::", "*"], token.value) || token.name === "punctuation" && token.value === "," || token.name === "ident" || token.name === "default") continue;
      return false;
    }
  ...
  function (context: AnalyzerContext): boolean {
    ...
    // like container.Resolve()
    {
      ...
      if (token && (token.name === "ident" || token.name === "operator" && token.value === ".")) return false;
    }
    ...
    while (walker.hasNext()) {
      ...
      if (token.name === "default" || token.name === "ident" || token.name === "punctuation" && token.value === ",") continue;
      return false;
    }
