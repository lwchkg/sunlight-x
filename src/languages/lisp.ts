...
const functionBoundary = "[\\s\\(\\)]";

/**
 * Create rule with function words and token type
 * @param {string[]} words
 * @param {string} tokenName
 * @returns {function}
 */
function createFunctionRule(words: string[], tokenName: string): (arg0: ParserContext) => Token | null | undefined {
  const map = util.createHashMap(words, functionBoundary, false);
  return function (context: ParserContext): Token | null | undefined {
    // must be right after a "("
    const token = util.getPreviousNonWsToken(context.getAllTokens(), context.count());
    if (!token) return null;
    if (token.name !== "punctuation" || token.value !== "(" && (token.name !== "operator" || token.value !== "#'")) return null;
    return util.matchWord(context, map, tokenName);
  };
}

export const name = "lisp";
export const scopes = {
  string: [['"', '"', [], false]],
  comment: [[";", "\n", [], true]],
  keywordArgument: [[":", {
...
export const customTokens = {
  ...
  keyword: {
    values: ["always", "appending", "append", "as", "collecting", "collect", "counting", "count", "doing", "do", "finally", "for", "if", "initially", "maximize", "maximizing", "minimize", "minimizing", "named", "nconcing", "nconc", "never", "repeat", "return", "summing", "sum", "thereis", "unless", "until", "when", "while", "with", // other loop keywords
    "into", "in"],
    boundary: "[^\\w-]"
  },
  globalVariable: {
...
export const customParseRules = [// #<n>r operator where n is an integer
...
function (context: ParserContext): Token | null | undefined {
  if (context.defaultData.text !== "" || /\s/.test(context.reader.peek())) // whitespace is not allowed
    return null;
  const prevToken = context.getAllTokens()[context.count() - 1];
  if (!prevToken || prevToken.name !== "operator" || prevToken.value !== "#\\") return null;
...
function (context: ParserContext): Token | null | undefined {
  if (context.reader.peek() !== "*") return null;
  const token = util.getPreviousNonWsToken(context.getAllTokens(), context.count());
  if (token && token.name === "punctuation" && token.value === "(") // function that starts with "*"
    return null;
...
function (): (arg0: ParserContext) => Token | null | undefined {
  ...
  return function (context: ParserContext): Token | null | undefined {
    if (context.defaultData.text !== "" || boundary.test(context.reader.peek())) // whitespace is not allowed or we're already at the boundary
      return null;
    const token = context.getAllTokens()[context.count() - 1];
    if (!token || token.name !== "operator" || token.value !== "#'") return null;
...
export const namedIdentRules = {
  custom: [function (): (arg0: AnalyzerContext) => boolean {
    ...
    return function (context: AnalyzerContext): boolean {
      ...
      const identValue = context.tokens[context.index].value;

      if (prevToken && prevToken.name === "macro" && util.contains(defMacros, prevToken.value)) {
...
export const operators = ["=>", "#B", "#b", "#O", "#o", "#X", "#x", "#C", "#c", "#S", "#s", "#P", "#p", "#.", "#:", "#\\", "#'", "#", "'", "...", "..", "."];