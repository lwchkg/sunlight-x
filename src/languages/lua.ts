...
export const keywords = ["and", "break", "do", "elseif", "else", "end", "false", "for", "function", "if", "in", "local", "nil", "not", "or", "repeat", "return", "then", "true", "until", "while"];
export const scopes = {
  string: [['"', '"', ['\\"', "\\\\"], false], ["'", "'", ["\\'", "\\\\"], false]],
  comment: [["--[[", "]]", [], false], ["--", "\n", [], true]]
};
...
export const customParseRules = [// standard functions
function (): (arg0: ParserContext) => Token | null | undefined {
  const functions = util.createHashMap(["assert", "collectgarbage", "dofile", "error", "getfenv", "getmetatable", "ipairs", "load", "loadfile", "loadstring", "next", "pairs", "pcall", "print", "rawequal", "rawget", "rawset", "select", "setfenv", "setmetatable", "tonumber", "tostring", "type", "unpack", "xpcall", // exported from package library
  "module", "require"], "\\b");
...
function (): (arg0: ParserContext) => Token | null | undefined {
  const functions = util.createHashMap(["close", "flush", "lines", "read", "seek", "setvbuf", "write"], "\\b");
  return function (context: ParserContext): Token | null | undefined {
    const walker = context.getTokenWalker();
...
export const namedIdentRules = {
  custom: [function (): (arg0: AnalyzerContext) => boolean {
    const tables = ["coroutine", "package", "string", "table", "math", "io", "os", "debug"];
