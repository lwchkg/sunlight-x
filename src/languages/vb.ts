...
import type { AnalyzerContext, ParserContext } from "../util";

/* eslint no-magic-numbers: 1 */
export const name = "vb";
export const keywords = ["AddHandler", "AddressOf", "Alias", "AndAlso", "And", "As", "Boolean", "ByRef", "Byte", "ByVal", "Call", "Case", "Catch", "CBool", "CByte", "CChar", "CDate", "CDbl", "CDec", "Char", "CInt", "Class", "CLng", "CObj", "Const", "Continue", "CSByte", "CShort", "CSng", "CStr", "CType", "CUInt", "CULng", "CUShort", "Date", "Decimal", "Declare", "Default", "Delegate", "Dim", "DirectCast", "Double", "Do", "Each", "ElseIf", "Else", "EndStatement", "EndIf", "End", "Enum", "Erase", "Error", "Event", "Exit", "False", "Finally", "ForEach", "For", "Friend", "Function", "GetType", "GetXMLNamespace", "Get", "Global", "GoSub", "GoTo", "Handles", "If", "Implements", "Imports", "Inherits", "Integer", "Interface", "In", "IsNot", "Is", "Let", "Lib", "Like", "Long", "Loop", "Me", "Module", "Mod", "MustInherit", "MustOverride", "MyBase", "MyClass", "Namespace", "Narrowing", "New", "Next", "Nothing", "NotInheritable", "NotOverridable", "Not", "Object", "Of", "On", "Operator", "Option", "Optional", "OrElse", "Or", "Out", "Overloads", "Overridable", "Overrides", "ParamArray", "Partial", "Private", "Property", "Protected", "Public", "RaiseEvent", "ReadOnly", "ReDim", "RemoveHandler", "Resume", "Return", "SByte", "Select", "Set", "Shadows", "Shared", "Short", "Single", "Static", "Step", "Stop", "String", "Structure", "Sub", "SyncLock", "Then", "Throw", "To", "True", "TryCast", "Try", "TypeOf", "UInteger", "ULong", "UShort", "Using", "Variant", "Wend", "When", "While", "Widening", "WithEvents", "With", "WriteOnly", "Xor"];
...
export const customParseRules = [DotNetCommon.XMLDocComment("'''"), // keyword escaping: e.g. "[In]"
util.getRegexpParser("escapedKeyword", new RegExp(/^\[[A-Za-z_]\w*\]/)), // handles New/GetType contextual keywords
...
function (): any {
  ...
  return function (context: ParserContext): Token | null | undefined {
    ...
    if (prevToken && (prevToken.name === "operator" && prevToken.value === "." || prevToken.name === "keyword" && prevToken.value === "Sub")) return new Token("ident", token.value, token.language);
    return token;
  };
}()];
...
export const namedIdentRules = {
  custom: [// attributes (copypasted (mostly) from c# file)
  function (context: AnalyzerContext): boolean {
    ...
    while (walker.hasPrev()) {
      ...
      } else if (token.name === "keyword" && util.contains(["Public", "Class", "Protected", "Private", "Friend"], token.value)) {
        break;
      }
    }
    ...
    while (walker.hasNext()) {
      ...
      } else if ( // End of named ident token
      token.name === "keyword" && util.contains(["Public", "Class", "Protected", "Private", "Friend", "ByVal"], token.value)) {
        break;
      }
    }
  ...
  function (context: AnalyzerContext): boolean {
    ...
    while (walker.hasPrev()) {
      ...
      if (token.name === "keyword") switch (token.value) {
        ...
        default:
          return false;
      } else if (token.name === "default" && token.value.indexOf(util.eol) >= 0) // apparently they must be on the same line...?
        return false;
    }
...
const numberLiteral: string = ((): string => {
  ...
  const nonDecIntegerLiteral = util.nonCapturingGroup([hex, oct, bin]) + intTypeChars + "?";
  const exponent = "E[+-]?" + int;
  const fpValue1 = util.nonCapturingGroup([util.nonCapturingGroup(int, "?") + "\\." + int + util.nonCapturingGroup(exponent, "?"), int + exponent]);
  ...
  const decIntegerLiteral = int + util.nonCapturingGroup([intTypeChars, fpTypeChars], "?");
