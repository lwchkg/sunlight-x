...
export const keywords = ["after", "andalso", "and", "band", "begin", "bnot", "bor", "bsl", "bsr", "bxor", "case", "catch", "cond", "div", "end", "fun", "if", "let", "not", "of", "orelse", "or", "query", "receive", "rem", "try", "when", "xor", "true", "false"];
export const customParseRules = [// atom/function/userDefinedFunction detection
function (context: ParserContext): Token | null | undefined {
  ...
  for (offset++; parenCount > 0; offset++) {
    const peek = context.reader.peekWithOffset(offset);
    if (peek === "") break;
    if (peek === "(") parenCount++;else if (peek === ")") parenCount--;
  }
