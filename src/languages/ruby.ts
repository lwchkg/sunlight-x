...
export const keywords = ["BEGIN", "END", "__ENCODING__", "__END__", "__FILE__", "__LINE__", "alias", "and", "begin", "break", "case", "class", "def", "defined?", "do", "else", "elsif", "end", "ensure", "false", "for", "if", "in", "module", "next", "nil", "not", "or", "redo", "rescue", "retry", "return", "self", "super", "then", "true", "undef", "unless", "until", "when", "while", "yield"];
...
export const customParseRules = [// regex literal, mostly the same as javascript
function (context: ParserContext): Token | null | undefined {
  ...
  if ( // The first token of the string
  previousToken !== undefined && ( // Since Ruby doesn't require statement terminators, if the previous token
...
function (context: ParserContext): Token | null | undefined {
  ...
  while (walker.hasPrev()) {
    ...
    if (token.name === "operator") {
      ...
      if (walker.hasPrev()) {
        ...
        if (prevToken.name === "operator" || prevToken.name === "punctuation" && prevToken.value === ",") // line continuation
          continue;
      }
  ...
  while (!context.reader.isEOF() && /\w/.test(context.reader.peek())) symbol += context.reader.read();
...
function (context: ParserContext): Token[] | null | undefined {
  ...
  while (Array.isArray(context.items.heredocQueue) && context.items.heredocQueue.length > 0 && !context.reader.isEOF()) {
    ...
    const lengthOfMatch = match ? match.index + match[0].length : substring.length;
...
function (context: ParserContext): Token | null | undefined {
  ...
  if (isRegex) // read potential regex modifiers
    while (!context.reader.isEOF()) {
...
function (context: ParserContext): Token | null | undefined {
  ...
  while (!context.reader.isEOF() && !context.reader.match(endOfDoc)) value += context.reader.read();
...
export const namedIdentRules = {
  ...
  [{
    token: "keyword",
    values: ["class", "def"]
  }, util.whitespace], // extended classes
  [{
    token: "keyword",
    values: ["class"]
  }, util.whitespace, {
  ...
  precedes: [// static variable access
  [util.whitespace, {
    token: "operator",
    values: ["::"]
  }], // new-ing a class
