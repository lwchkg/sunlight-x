...
export const customParseRules = [// idents and special operators
...
function (): (arg0: ParserContext) => Token | null | undefined {
  const specialOperators = ["-not", "-band", "-bor", "bnot", "-replace", "-ireplace", "-creplace", "-and", "-or", "-isnot", "-is", "-as", "-F", "-lt", "-le", "-gt", "-ge", "-eq", "-ne", "-contains", "-notcontains", "-like", "-notlike", "-match", "-notmatch"];
  const keywords = [// [type]::gettype("System.Management.Automation.KeywordTokenReader")|%{$_.InvokeMember("_keywordTokens", "NonPublic,Static,GetField", $null, $_,@())}
  ...
  return function (context: ParserContext): Token | null | undefined {
    ...
    const tokenType = util.contains(specialOperators, ident) ? "specialOperator" : util.contains(keywords, ident) ? "keyword" : ident.charAt(0) === "-" ? "switch" : "ident";
...
function (): (arg0: ParserContext) => Token | null | undefined {
  ...
  const specialVariables = ["$$", "$?", "$^", "$_", "$ARGS", "$CONSOLEFILENAME", "$ERROR", "$EVENT", "$EVENTSUBSCRIBER", "$EXECUTIONCONTEXT", "$FALSE", "$FOREACH", "$HOME", "$HOST", "$INPUT", "$LASTEXITCODE", "$MATCHES", "$MYINVOCATION", "$NESTEDPROMPTLEVEL", "$NULL", "$PID", "$PROFILE", "$PSBOUNDPARAMETERS", "$PSCMDLET", "$PSCULTURE", "$PSDEBUGCONTEXT", "$PSHOME", "$PSSCRIPTROOT", "$PSUICULTURE", "$PSVERSIONTABLE", "$PWD", "$SENDER", "$SHELLID", "$SOURCEARGS", "$SOURCEEVENTARGS", "$THIS", "$TRUE"];
  ...
  return function (context: ParserContext): Token | null | undefined {
    ...
    while (!context.reader.isEOF() && !invalidVariableCharRegex.test(context.reader.peek())) value += context.reader.read();
...
export const namedIdentRules = {
  ...
  function (context: AnalyzerContext): boolean {
    ...
    return util.IsBetweenRuleSatisfied(context.getTokenWalker(), {
      token: "punctuation",
      values: ["["]
    }, {
      token: "punctuation",
