...
import type { ArrayWalker } from "./array-walker.js";
import type { FollowsOrPrecedesIdentRule } from "./languages.js";
 ...
 * @returns {boolean}
 */
export function IsPrecedesRuleSatisfied(
  walker: ArrayWalker<Token>,
 ...
 * @returns {boolean}
 */
export function IsFollowsRuleSatisfied(
  walker: ArrayWalker<Token>,
  tokenRequirements: FollowsOrPrecedesIdentRule,
  caseInsensitive: boolean = false
): boolean {
 ...
 * @returns {boolean}
 */
export function IsBetweenRuleSatisfied(
  walker: ArrayWalker<Token>,
  ...
  while (walker1.hasPrev()) {
    ...
    if (
      ...
      (!opener.values || contains(opener.values, token.value, caseInsensitive))
    ) {
      success = true;
      break;
    }
    ...
    if (
      ...
      (!closer.values || contains(closer.values, token.value, caseInsensitive))
    )
      return false;
  }
