...
import { getCSSSync, Highlighter } from "../src/sunlight";
import { getOptionsFromString, TestSupportForFile } from "./fixtures/testsupport";
...
function getHighlightLanguage(filename: string): string {
  ...
  assert.notStrictEqual(language, "", "Cannot find the language of extension " + extension);
  return language;
}
...
function compareResult(filename: string, actualContent: string) {
  ...
  const expectedFilename = path.join(resultsDir, fileData.name + "-expected" + fileData.ext);

  if (fs.existsSync(expectedFilename)) {
    const expectedContent = fs.readFileSync(expectedFilename, "utf8");
    if (expectedContent === actualContent) return;
    const actualFilename = path.join(resultsDir, fileData.name + "-actual" + fileData.ext);

    if (expectedContent !== actualContent) {
      fs.writeFileSync(actualFilename, actualContent, "utf8");
      throw new Error(`Content doesn't match. \`${actualFilename}\` written for comparison.`);
    }
  } else {
    fs.writeFileSync(expectedFilename, actualContent, "utf8");
    throw new Error(`Test expectation file \`${expectedFilename}\` does not exist. New expectation written. Please commit the file.`);
  }
}
...
describe("HTML files generation test", function () {
  ...
  it("Renders the demo page correctly", function () {
    ...
    const generatedHTML = HTMLTemplate.replace(/%%([A-Z][A-Z0-9]*)%%/gi, (match: string, p1: string): string => {
