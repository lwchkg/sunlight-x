...
describe("API tests", function () {
  it("all highlight functions have equivalent output", function () {
    ...
    Highlighter.resetNodeCount();
    const highlightedElement = highlighter.highlightCodeAsElement(code, language);
  ...
  it("respects theme settings", function () {
    const code = 'console.log("test")\nconsole.log("test")';
    const language = "javascript";
    const customHighlighter = new Highlighter({
      theme: "abcd"
    });
    const classPrefix = customHighlighter.options.classPrefix;
    const highlightedElement = customHighlighter.highlightCodeAsElement(code, language);
    const actualThemeList = highlightedElement.className.split(" ").filter((item: string): boolean => item.startsWith(classPrefix + "theme-"));
