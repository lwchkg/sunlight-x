...
import { djb2Utf8 } from "./fixtures/djb2-utf8";
import { getOptionsFromString, getOptionsFromHash } from "./fixtures/testsupport";
describe("Test of test fixtures", function () {
  ...
  describe("getOptionsFromString", function () {
    it("returns a set of options", function () {
      const s = "test";
      assert.deepEqual(getOptionsFromString(s), getOptionsFromHash(djb2Utf8(s)));
    });
  });
    ...
    it("has the correct peroidic behavior", function () {
      ...
      for (let hash = 0; hash < count; ++hash) {
        ...
        for (let index = 0; index < cardinality; ++index) {
          ...
          if (typeof expected[index][counts[index]] === "undefined") expected[index][counts[index]] = actual;else assert.strictEqual(actual, expected[index][counts[index]], `Failed on hash = ${hash}, field ${periods[index].name}`);
        }
